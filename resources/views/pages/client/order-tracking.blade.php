    ...
    <main class="w-full min-h-screen bg-gradient-to-b from-red-50 to-white py-10 px-4 lg:px-30">
        <div class="max-w-4xl mx-auto">
                        ...
                        <div class="relative flex justify-between">
                            @foreach ($statusSteps as $index => $step)
                                @php
                                    $isActive = in_array(
                                        $step,
                                        array_slice($statusSteps, 0, array_search($currentStatus, $statusSteps) + 1),
                                    );
                                    $isCurrent = $step === $currentStatus;
                                @endphp
                                <div class="flex flex-col items-center" style="width: {{ 100 / count($statusSteps) }}%">
                                    <div
                                        class="w-12 h-12 rounded-full flex items-center justify-center relative z-10 {{ $isActive ? 'bg-red-500' : 'bg-gray-200' }}">
                                        @if ($isActive)
                                    ...
                                    <div class="mt-3 text-center">
                                        <div
                                            class="font-semibold text-sm {{ $isActive ? 'text-red-600' : 'text-gray-500' }}">
                                            {{ ucfirst($step) }}
                                        </div>
                                        @if ($isCurrent)
                    ...
                    <div class="space-y-4">
                        @foreach ($order->items as $item)
                            <div class="flex gap-3">
                                <div
                                    class="w-16 h-16 bg-gradient-to-b from-red-100 to-red-200 rounded-lg flex items-center justify-center flex-shrink-0">
                        ...
                        @if ($order->shippingMethod)
                            <div>
                                ...
                                @if ($order->shippingMethod->estimated_days)
                                    <p class="text-sm text-gray-500">Estimated:
            ...
            <div class="flex gap-4">
                <a href="{{ route('orders') }}"
                    class="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-4 rounded-lg transition-colors text-center text-lg">
                    Back to Orders
                </a>
                @if ($order->status == 'pending')
                    <form action="{{ route('orders.cancel', $order) }}" method="POST" class="flex-1">
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="w-full bg-gray-600 hover:bg-gray-700 text-white font-bold py-4 rounded-lg transition-colors text-lg">
                            Cancel Order
                        </button>
                    </form>
                @endif
                ...
                <div class="bg-white rounded-xl shadow-lg p-8 mt-8">
                    <div class="flex items-start gap-4 mb-6">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-gray-600">If you're experiencing any problems with your order, you can create a
                                dispute and our support team will help resolve it.</p>
                        </div>
                    </div>

                    <button onclick="document.getElementById('dispute-modal').classList.remove('hidden')"
                        class="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-lg transition-colors text-lg">
                    ...
                    <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div class="p-6 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <h3 class="text-2xl font-bold text-gray-800">Create Dispute for Order #{{ $order->id }}
                                </h3>
                                <button onclick="document.getElementById('dispute-modal').classList.add('hidden')"
                        ...
                        <form action="{{ route('orders.dispute', $order) }}" method="POST" class="p-6">
                            ...
                            <div class="bg-orange-50 border border-orange-200 rounded-lg p-4 mb-6">
                                <p class="text-sm text-orange-800">
                                    ...
                                    <strong>Note:</strong> Once submitted, our support team will review your dispute and
                                    contact you within 24-48 hours. Please provide as much detail as possible.
                                </p>
                            </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-xl shadow-lg p-6 mt-8">
                    <div class="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-blue-700">A dispute has already been created for this order. Our team is
                                reviewing it and will contact you soon.</p>
                            <p class="text-sm text-blue-600 mt-2">Status:
                                <strong>{{ ucfirst(str_replace('_', ' ', $order->dispute->status)) }}</strong></p>
                        </div>
                    </div>
                </div>
            @endif
